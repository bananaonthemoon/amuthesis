...
```{r tabela31, echo=FALSE, fig.cap="Fragment danych z II tury pobranych z wybory.gov.pl"}
library(knitr)
library(kableExtra)
library(magrittr)
dane_woj = read.csv2("data/tabela31.csv", encoding = "UTF-8")

kable(head(dane_woj[c(1:5)], 5), booktabs = TRUE, col.names = c('TERYT', 'Województwo', 'Wyborcy uprawnieni', 'Andrzej Duda', 'Rafał Trzaskowski')) %>%
...
```{r tabela32, echo=FALSE, fig.cap="Fragment danych po etapie czyszczenia oraz łączenia"}
library(knitr)
library(kableExtra)
dane_woj = read.csv2("data/tabela_2.csv", encoding = "UTF-8")
kable(head(dane_woj[c(1,6,8, 9)], 5), booktabs = TRUE)
```
...
```{r tabela33, echo=FALSE, fig.cap="TABLEAAAA????"}
library(knitr)
library(kableExtra)
dane_woj = read.csv2("data/tabela_2.csv", encoding = "UTF-8")
kable(head(dane_woj[c(1,6,8, 9)], 5), booktabs = TRUE)
...
Innymi pakietami użytymi były m.in.: dplyr, sf, tmap oraz tmaptools.
...
Manipulując argumentem *k* mamy wpływ na udział obwiedni x wypełnionej większym okręgiem, a argumentem - *m_weight*, czyli wagi ruchomości okręgów. 
