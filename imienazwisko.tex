...
\def\titleeng{2020 presidential election results in Poland - comparison of spatial visualisation techniques}
\def\degreetitle{Praca inżynierska}
\def\major{YORKETOOOON}
\def\albumid{431977}
\def\thesisyear{2021}
...
Odniesieniem dla zjawisk jest powierzchnia pola podstawowego/jednostki przestrzennej, którą wykorzystujemy \autocite{ratajski1989}.
...
Ten drugi jest znacznie rzadziej wykorzystywany \autocite{ratajski1989}.
...
Chociaż kartogram jest powszechnie używany, jest on często nieadekwatny i prowadzi do mylnej interpretacji \autocite{leonowicz2006}.
...
\caption{Mapa wyborów prezydenckich w Stanach Zjednoczonych w 2020 roku. Źródło: \url{https://commons.wikimedia.org/wiki/File:United_States_presidential_election_results_by_county,_2020.svg}}\label{fig:ryc21}
\end{figure}
...
Kartogramy geometryczne są wyjątkowo przydatne przy porównaniach pomiędzy latami, kiedy istnieje szansa, że granice jednostek się zmieniły \autocite{ratajski1989}.
...
\caption{Mapy geometryczne (heksagonalna oraz kwadratowa) na przykładzie Stanów Zjednoczonych. Źródło: https://blog.apps.npr.org/2015/05/11/hex-tile-maps.html}\label{fig:ryc22}
\end{figure}
...
Popularność tego rodzaju kartogramów jest niska ze względu na trudności, które odbiorca napotyka \autocite{lamparski2018}. W polskiej literaturze pojawiają się one w kilku wariantach: jako kartogramy eumorficzne lub odwrócone \autocite{ratajski1989}, mapy anamorficzne, które następnie dzielone są na mapy anamorficzne styczne i niestyczne \autocite{zyszkowska2012} lub ciągłe i nieciągłe (najczęściej używane w artykułach naukowych, np. \textcite{lamparski2018}, tłum. z języka angielskiego -- \emph{contiguous} oraz \emph{non contiguous}). Warto zaznaczyć, że w języku angielskim kartogram nazywany jest choropleth map i nie należy go mylić z metodą nazwaną cartogram, która odnosi się do map anamorficznych. W tej pracy używane jest pojęcie mapa anamorficzna, mapa anamorficzna ciągła lub nieciągła.
...
\caption{Cartogram Cube - bryła przedstawiająca właściwości anamorfoz powierzchniowych. Anamorfozy ciągłe zaznaczono na niebiesko, na zielono - anamorfozy nieciągłe, na fioletowo - tradycyjną mapę, a na czerwono - idealną anamorfozę. Źródło: Rycina powstała na podstawie artykułu R.E. Roth i in. 2010 (Markowska, 2019)}\label{fig:ryc23}
\end{figure}

Własności anamorfoz została podsumowana na Rycinie \ref{fig:ryc23} \autocite{roth2010}.
Przedstawia ona \emph{Cartogram Cube} - jest to graficzna metoda przedstawiania zależności między cechami wpływającymi na atrybuty informacyjne map \autocite{markowska2019}.
...
Rezultatem tego są opracowane algorytmy, m.in. Gastnera i Newmana \autocite{gastner2004}, Dougenika \autocite{dougenik1985} oraz Gusein-Zade'a i Tikunova \autocite{guseinzade1993}. Różnią się one m.in. stopniem zachowania kształtu oraz poprawnością topologii.
...
\caption{Anamorfoza projekcji podczas wyborów prezydenckich w USA. Wielkość każdego stanu jest zgodna z liczbą głosów. Źródło: https://www.reddit.com/r/dataisbeautiful/comments/jomi6h/}\label{fig:ryc24}
\end{figure}

W anamorfozie nieciągłej kształt poszczególnych jednostek zostaje zachowany, a rozmiar powierzchni jest modyfikowany w zależności od wartości zjawiska \autocite{olson1976}.
...
Jest to powszechnie stosowana forma wizualizacji w kartografii brytyjskiej \autocite{dorling1996}.
Technika ta zastępuje rzeczywisty kształt jednostek na rzecz wyeksponowania wartości zjawiska za pomocą kół lub rzadziej heksagonów \autocite{faliszewska2010}.
...
Porównując mapy anamorficzne z klasycznymi metodami jak kartogramy i kartodiagramy, można zauważyć, że takie wizualizacje wydają się być lepsze z punktu widzenia semiotycznego, jako że kształt pola odniesienia zmieni się wraz wartością zmienną \autocite{krzywickablum2009}.
...
Praca została oparta o dane ze strony internetowej Państwowej Komisji Wyborczej, z zakładki dotyczącej wyborów prezydenckich RP w czerwcu 2020 roku \autocite{pkw_2020}.
Na stronie dostępne są dane w rozszerzeniach \emph{.csv} oraz \emph{.xlsx} -- w pracy skorzystano z plików z rozszerzeniem .csv ze względu na łatwość ich obsługi w środowisku \emph{R}.
...
\caption{\label{tab:tabela31}Fragment surowych danych wyborczych z II tury Źródło: opracowanie własne na podstawie danych PKW}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:tabela32}Fragment wyczyszczonych danych wyborczych z II tury Źródło: opracowanie własne na podstawie danych PKW}
\centering
\resizebox{\linewidth}{!}{
...
Dane z granicami administracyjnymi uzyskano ze strony GIS Support \autocite{gissupport}.
...
Każdy plik .shp posiada kolumny \emph{`JPT\_SJR\_KO'}, \emph{`JPT\_KOD\_JE'} oraz \emph{`JPT\_NAZWA\_'}. Kolumna \emph{`JPT\_KOD\_JE'} jest niezbędna do przeprowadzenia pomyślnego łączenia danych geograficznych z danymi wyborczymi.

Czyszczenie danych zostały przeprowadzone w języku R \autocite{R-base}, konieczne jednak były poprawki geometrii na poziomie powiatowym -- naprawiono je w programie QGIS.
...
Za pomocą wspólnej kolumny \emph{TERYT} złączono obie tury.
...
Poprzez kolumny \emph{TERYT} złączono dane tekstowe z danymi geograficznymi do jednego pliku.
...
Złączony plik zapisano w formacie GeoPackage (rozszerzenie \emph{.gpkg}).

\begin{table}

\caption{\label{tab:tabela33}Fragment wyczyszczonych danych geograficznych oraz danych wyborczych z II tury Źródło: opracowanie własne na podstawie danych PKW}
\centering
\resizebox{\linewidth}{!}{
...
Głównym narzędziem stosowanym w pracy inżynierskiej był język \emph{R}.
Łącznie użytych zostało 9 pakietów, z czego najważniejszymi były pakiety \emph{cartogram} oraz \emph{geogrid}, które umożliwiały wizualizację danych przestrzennych (Tab. \ref(tab:tabela34)).
Innymi ważnymi pakietami użytymi w pracy były m.in.: dplyr \autocite{r-dplyr}, sf \autocite{r-sf}, tmap \autocite{r-tmap} oraz tmaptools \autocite{r-tmaptools}.

\begin{table}

\caption{\label{tab:tabela34}Spis użytych bibliotek języka R na każdym etapie tworzenia pracy. Źródło: opracowanie własne}
\centering
...
Pakiet \emph{cartogram} składa się z trzech funkcji: \emph{cartogram\_cont(), cartogram\_ncont(), oraz cartogram\_dorling()}.
\emph{Cartogram\_cont()} służy do tworzenia kartogramu anamorficznego ciągłego za pomocą algorytmu zniekształcenia arkusza gumy - gubbersheetingu \autocite{dougenik1985}.
\emph{Cartogram\_ncont()} tworzy kartogram anamorficzny nieciągły, którego algorytm jest oparty o pracę J. Olson \autocite{olson1976}.
Przy pomocy argumentu \emph{k} możliwe jest określenie współczynnika ekspansji dla jednostek o większej wartości.
\emph{Cartogram\_dorling()} tworzy kartodiagram anamorficzny, opracowany pierwotnie przez D. Dorlinga \autocite{dorling1996}.
Wizualizacje z tego pakietu tworzone były w połączeniu z pakietem \emph{tmap.}
Manipulując argumentem \emph{k} mamy wpływ na udział obwiedni jednostki wypełnionej większym okręgiem, a argumentem \emph{m\_weight} - wagę ruchomości kół, tj. ustalając w argumencie \emph{m\_weight} wartość 0 uniemożliwiamy ruch jednostkek z przypisanego miejsca. Wartość 1 jest domyślną odległością ruchomości jednostki od przypisanego miejsca obliczoną przez algorytm.

W pakiecie \emph{geogrid} przy pomocy funkcji \emph{calculate\_grid()} nadawany jest kształt pola odniesienia (tj. heksagon, kwadrat). Następnie generowane są siatki pól (gridy), które oparte są o analizy granic oryginalnych jednostek odniesienia. Kolejnym etapem jest przypisanie gridu do oryginalnych granic za pomocą funkcji \emph{Assign\_polygons()}. Ostatnim krokiem jest zwizualizowanie danych przy pomocy pakietu tmap.

\hypertarget{wizualizacje}{%
\chapter{Wizualizacje}\label{wizualizacje}}

W tym rozdziale przedstawione zostały przykładowe mapy wyborcze używając technik wizualizacji, które można stworzyć za pomocą bibliotek \emph{geogrid} oraz \emph{cartogram.}
...
W Atlasie Rzeczypospolitej Polskiej kartogram zastosowano dla prawie 80\% map, a w Narodowym Atlasie Polski jest to 64\% map \autocite{debowska2010}.
...
\caption{Przykład kartogramu właściwego na poziomie województw - poparcie dla Andrzeja Dudy w I turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc41}
\end{figure}
...
\caption{Przykład kartogramu właściwego na poziomie gmin w województwie wielkopolskim - poparcie dla Rafała Trzaskowskiego w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc42}
\end{figure}
...
\caption{Przykład kartogramu ciągłego na poziomie powiatów - frekwencja w I turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc43}
\end{figure}

\hypertarget{kartogramygeometryczne}{%
\section{Kartogramy geometryczne}\label{kartogramygeometryczne}}

W polskiej literaturze kartogramy geometryczne są najczęściej tworzone poprzez podzielenie całego badanego obszaru na pola o równym kształcie i wielkości \autocite{ratajski1989}.
...
\caption{Przykład kartogramu geometrycznego na poziomie województw - poparcie dla Rafała Trzaskowskiego w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc44}
\end{figure}
...
\caption{Przykład kartogramu geometrycznego na poziomie gmin w województwie wielkopolskim - wzrost frekwencji w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc45}
\end{figure}

\hypertarget{anamorfozaciagla}{%
\section{Anamorfoza ciągła}\label{anamorfozaciagla}}

Poniższa technika wizualizacji jest przedmiotem badań kartografów w Polsce, jednak nie zyskała popularności poza tym kręgiem \autocite{krzywickablum2009} \autocite{markowska2018} \autocite{markowska2019} \autocite{lamparski2018}.
...
\caption{Przykład anamorfozy ciągłej na poziomie województw - wzrost poparcia dla Andrzeja Dudy w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc46}
\end{figure}
...
\caption{Przykład kartogramu geometrycznego na poziomie gmin w województwie wielkopolskim - różnica głosów między kandydatami w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc47}
\end{figure}
...
\caption{Przykład anamorfozy ciągłej na poziomie województw - różnica głosów między kandydatami w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc48}
\end{figure}
...
\caption{Przykład kartogramu geometrycznego na poziomie gmin w województwie wielkopolskim - frekwencja w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc49}
\end{figure}
...
\caption{Przykład anamorfozy ciągłej na poziomie województw - wzrost poparcia dla Andrzeja Dudy w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc410}
\end{figure}
...
\caption{Przykład kartogramu geometrycznego na poziomie gmin w województwie wielkopolskim - poparcie dla Rafała Trzaskowskiego w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc411}
\end{figure}
...
\caption{Przykłady kartogramu geometrycznego na poziomie powiatów z trzema wariantami argumentu k - różnica głosów między kandydatami w II turze wyborów.Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc412}
\end{figure}
...
\textbf{Pytanie: Jaki odsetek powiatów miało frekwencję powyżej 60\%?}
...
\caption{Anamorfoza ciągla oraz kartogram ciągły - frekwencja w II turze wyborów. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc51}
\end{figure}

\textbf{Poprawna odpowiedź: 66\%}
...
Wyniki w grupach były podobne - w każdej z grup dwie osoby uważały, że \emph{mniej niż 20\%} powiatów miało wysoką frekwencję.
Niewiele więcej osób wybrało odpowiedź, iż wynik oscyluje \emph{między 20 a 40\%} - były to cztery osoby w pierwszej grupie, w drugiej sześć osób (Ryc. \ref{fig:ryc52}).
W pierwszej grupie najwięcej osób (14 os.) wybrało wariant \emph{40-60\%}, w drugiej grupie było to 11 osób.
Poprawną odpowiedź, czyli \emph{60-80\%} wybrało po 12 osób w każdej z grup.
Tylko dwie osoby z grupy pierwszej, a trzy z drugiej uważały, że odsetek jest \emph{wyższy niż 80\%}.
...
\caption{Wykres z wynikami do pytania pierwszego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc52}
\end{figure}

\hypertarget{pyt2}{%
\subsection{Pytanie drugie}\label{pyt2}}

\textbf{Pytanie: W jakim \% gmin kandydat B wygrał w II turze?}
...
\caption{Kartodiagram Dorlinga oraz kartogram geometryczny - poparcie dla kandydata B w II turze. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc53}
\end{figure}

\textbf{Poprawna odpowiedź: 36,3\%}
...
Na podstawie rycin w grupie drugiej dziewięć osób uważało, że kandydat B zdobył \emph{pomiędzy 20 a 30\%} głosów, w pierwszej grupie było to 11 osób (Ryc. \ref{fig:ryc54}).
Czwarty wariant, czyli \emph{40-50\%}, wybrało pięć osób w pierwszej i osiem osób w drugiej grupie.
Sześć osób z pierwszej oraz jedna osoba z drugiej grupy wywnioskowała, że kandydat B wygrał \emph{ponad 50\%} gmin w II turze wyborów.
...
\caption{Wykres z wynikami do pytania drugiego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc54}
\end{figure}

\hypertarget{pyt3}{%
\subsection{Pytanie trzecie}\label{pyt3}}

\textbf{Pytanie: W ilu \% gmin zaobserwowano wzrost frekwencji w II turze o co najmniej 5\%?}
...
\caption{Kartogram właściwy oraz kartogram geometryczny - wzrost frekwencji w II turze. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc55}
\end{figure}

\textbf{Poprawna odpowiedź: 21,6\%}
...
Niewiele mniej, czyli 14 osób wybrało odpowiedź \emph{15-20\%}. Wariant trzeci, czyli \emph{10-15\%} wybrało osiem osób, a \emph{5-10\%} - 11 osób ankietowanych (Ryc. \ref{fig:ryc56}).
Nikt w grupie pierwszej nie sądził, że wzrost frekwencji zaobserwowano w \emph{mniej niż 5\%} gmin.

W grupie drugiej tą samą opcję wybrała tylko jedna osoba, a sześć osób wybrało przedział \emph{5-10\%}.
Wariant \emph{10-15\%} wybrało najwięcej ankietowanych, czyli 12 osób.
11 osób zinterpretowało na mapie, że znaczny wzrost frekwencji zaobserwowano w \emph{15-20\%} gmin.
Tylko cztery osoby najwyższą opcję, czyli \emph{\textgreater{} 20\%}.
...
\caption{Wykres z wynikami do pytania trzeciego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc56}
\end{figure}

\hypertarget{pyt4}{%
\subsection{Pytanie czwarte}\label{pyt4}}

\textbf{Wymień 3 województwa z najwyższą frekwencją}
...
\caption{Kartogram geometryczny oraz anamorfoza nieciągła - frekwencja w II turze. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc57}
\end{figure}

\textbf{Poprawna odpowiedź: woj. mazowieckie, woj. małopolskie, woj. pomorskie}
...
Najczęstszą odpowiedzią drugiej grupy było \emph{woj. mazowieckie, woj. pomorskie, woj. wielkopolskie}.
W pierwszej grupie poza prawidłową odpowiedzią, popularna była również odpowiedź \emph{woj. mazowieckie, woj. pomorskie, woj. śląskie} (sześć osób) oraz \emph{woj. małopolskie, woj. mazowieckie, woj. zachodniopomorskie} (pięć osób).
...
\caption{Wykres z wynikami do pytania czwatego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc58}
\end{figure}

\hypertarget{pyt5}{%
\subsection{Pytanie piąte}\label{pyt5}}

\textbf{Pytanie: Jaki ogólnie \% głosów kandydat A uzyskał w woj. wielkopolskim?}
...
\caption{Anamorfoza ciągła oraz kartogram właściwy - poparcie dla kandydata A w I turze. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc59}
\end{figure}

\textbf{Poprawna odpowiedź: 37,8\%}
...
Żadna z grup nie uważała, że poparcie było \emph{mniejsze niż 35\%}. Osiem osób uważało, że wynik wynosił \emph{między 35 a 42\%} - były to dwie osoby w grupie pierwszej oraz sześć w grupie drugiej (Ryc. \ref{fig:ryc510}).
...
W przedziale \emph{40-50\%} było za to odwrotnie - połowa grupy drugiej wybrała ten przedział, a z grupy pierwszej - 13 osób.
Dwie osoby z każdej z grup uważało, że kandydat B uzyskał \emph{ponad 50\% głosów} w województwie wielkopolskim.
...
\caption{Wykres z wynikami do pytania piątego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc510}
\end{figure}

\hypertarget{pyt6}{%
\subsection{Pytanie szóste}\label{pyt6}}

\textbf{Pytanie: O ile średnio \% wzrosło poparcie dla kandydata B w II turze w porównaniu do I tury?}
...
\caption{Kartogram geometryczny oraz anamorfoza ciągła - wzrost poparcia dla kandydata B w II turze. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc511}
\end{figure}

\textbf{Poprawna odpowiedź: 17,6\%}

Na pytanie piąte udało się odpowiedzieć poprawnie obu grupom (Ryc. \ref{fig:ryc511}).
Ponad 50\% grupy pierwszej (18 osób) oraz 12 osób z grupy drugiej poprawnie wywnioskowało, że poparcie kandydata B w II turze wzrosło o \emph{mniej niż 20\%} (Ryc. \ref{fig:ryc512}).
Dziewięć osób z każdej z grup wybrało odpowiedź, że poparcie wzrosło \emph{między 20 a 40\%}.
Pięć osób z pierwszej grupy oraz dziesięć z drugiej grupy uważa, że odpowiedź \emph{40-60\%} była poprawna.
Przedostatnią odpowiedź, wybrały dwie osoby z pierwszej oraz trzy osoby z drugiej grupy.
Żadna z grup nie uważała, że poparcie wzrosło o \emph{więcej niż 80\%}.
...
\caption{Wykres z wynikami do pytania szóstego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc512}
\end{figure}

\hypertarget{pyt7}{%
\subsection{Pytanie siódme}\label{pyt7}}

\textbf{Pytanie: Mapa przedstawia różnicę głosów między kandydatami w II turze. W którym z województw była najmniejsza różnica wyników?}
...
\caption{Anamorfoza nieciągła oraz anamorfoza ciągła - róznica głosów między kandydatami w II turze. Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc513}
\end{figure}

\textbf{Poprawna odpowiedź: woj. opolskie}
...
W obu grupach największą liczbę głosów zdobyło \emph{województwo opolskie} - zostałaona wybrana przez 11 osób z grupy pierwszej oraz dziewięć z grupy drugiej (Ryc. \ref{fig:ryc514}).
Drugim najczęściej wybieranym województwem było \emph{województwo podkarpackie} - sześć osób w każdej z grup uważało, że ta odpowiedź jest poprawna.
W grupie pierwszej \emph{województwo mazowieckie} oraz \emph{kujawsko-pomorskie} uzyskało po cztery głosy.
Pięć osób z drugiej grupy wybrało \emph{województwo zachodniopomorskie}, a następne pięć - \emph{lubelskie}.
...
\caption{Wykres z wynikami do pytania siódmego (obie grupy). Źródło: opracowanie własne na podstawie danych PKW}\label{fig:ryc514}
\end{figure}
...
Bazując na tym wyniku oraz zasadzie kartograficznej, która nie zaleca używania więcej niż siedem lub osiem podobnych odcieni \autocite{ratajski1989}, można wyciągnąć wniosek, że używanie wielu gradacji jednej barwy dla dużej liczby jednostek odniesienia znacznie utrudnia użytkownikowi odczytanie prawidłowych informacji.
...
W pytaniu, gdzie użyto kartogramu heksagonalnego dla gmin województwa wielkopolskiego, nie uzyskano jednoznacznej odpowiedzi u ankietowanych - 98\% głosów zostało podzielonych między trzy odpowiedzi, w zakresie od \emph{\textless{} 20\%} do \emph{40-50\%}.
...
Warianty \emph{\textless{} 20\%, 20-40\%} i \emph{40-60\%} różniły się między sobą jednym lub dwoma głosami.
Całkowicie inaczej było w pytaniu 5, gdzie znaczna część ankietowanych wybrała poprawny zakres, w mniejszym stopniu ankietowani wybrali sąsiadujący zakres (poprawna odpowiedź wynosiła 37,8\%, a przedziały dzieliły się na zakresy \emph{30-40\%} oraz \emph{40-50\%}, przez co można podejrzewać, że ta mniejsza część ankietowanych nieznacznie przeszacowała wynik).
...
Pomimo tego, tylko 30,6\% osób wybrało prawidłową odpowiedź, a 17,6\% wybrało \emph{województwo podkarpackie}, które było jednostką z największą różnicą głosów.
Sugeruje to, że część ankietowanych mogło źle zrozumieć pytanie, tak jak w przypadku anamorfozy ciągłej.

Z zadania, w którym został użyty kartodiagram Dorlinga wynika, że 23\% ankietowanych odpowiedziało poprawnie i wybrało odpowiedź \emph{30-40\%}.
Nie była to jednak najczęściej wybierana odpowiedź (jest to \emph{20-30\%} wybrane przez 11 os.).
...
Za to w podrozdziale \ref{pyt5} połowa ankietowanych odpowiedziała poprawnie (odpowiedź \emph{30-40\%}), na sąsiedni wariant \emph{40-50\%} zagłosowało 38,2\%.
